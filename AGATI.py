...
class Window(wx.Frame):
    ...
    def __init__(self):
        ...
        wx.Frame.__init__(
            self, None, id=wx.ID_ANY, title="AGATI", pos=(100, 100), size=(x / 2, y / 2)
        )
    ...
    def file_select(self) -> (any, bool):
        ...
        dlg = wx.MessageBox(
            "Would you like to analyze a new video?", "Confirm", wx.YES_NO
        )
        if dlg == wx.YES:
            dlg = wx.MessageBox(
            ...
            dlg = wx.MessageBox(
                "Would you like to close the program?", "Confirm", wx.YES_NO
            )
            if dlg == wx.YES:
                self.quit()
            else:
